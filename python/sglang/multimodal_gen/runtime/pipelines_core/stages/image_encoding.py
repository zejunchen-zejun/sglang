...
class ImageEncodingStage(PipelineStage):
    ...
    def _maybe_compile_text_encoder(self):
        ...
        if not self.server_args.enable_torch_compile:
            logger.info("[DEBUG] torch.compile is disabled, skipping text encoder compilation")
            return

        if self.text_encoder is None:
            logger.info("[DEBUG] text_encoder is None, skipping compilation")
            return

        logger.info(
            f"[DEBUG] ImageEncodingStage._maybe_compile_text_encoder: "
        ...
        logger.info(f"[DEBUG] text_encoder is_fsdp={is_fsdp}")
