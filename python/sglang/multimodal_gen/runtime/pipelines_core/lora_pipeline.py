    ...
    def _apply_lora_to_layers(
        ...
        for name, layer in lora_layers.items():
            ...
            if (
                ...
                try:
                    inferred_rank = int(
                        self.lora_adapters[lora_nickname][lora_A_name].shape[0]
                    )
                except Exception:
                    inferred_rank = None
                ...
                if alpha_key in self.lora_adapters[lora_nickname]:
                    try:
                        inferred_alpha = int(
                            self.lora_adapters[lora_nickname][alpha_key].item()
                        )
                    except Exception:
                        inferred_alpha = None
                ...
                if inferred_rank is not None:
                    ...
                    layer.lora_alpha = (
                        inferred_alpha if inferred_alpha is not None else inferred_rank
                    )
