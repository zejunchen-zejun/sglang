...
async def start_profile(request: Request, obj: Optional[ProfileReqInput] = None):
    ...
    try:
        ...
        env_with_stack = os.getenv("SGLANG_PROFILE_WITH_STACK", "false").lower() in ("true", "1", "yes")
        env_record_shapes = os.getenv("SGLANG_PROFILE_RECORD_SHAPES", "false").lower() in ("true", "1", "yes")
        
        with_stack = obj.with_stack if obj.with_stack is not None else env_with_stack
        record_shapes = obj.record_shapes if obj.record_shapes is not None else env_record_shapes
        ...
        from sglang.multimodal_gen.runtime.entrypoints.openai.utils import StartProfileReq
        ...
        logger.info(
            f"Profilers started with profile_id={profile_id}. "
            f"Output: {output_dir}"
        )
...
async def stop_profile():
    ...
    try:
        ...
        from sglang.multimodal_gen.runtime.entrypoints.openai.utils import StopProfileReq
