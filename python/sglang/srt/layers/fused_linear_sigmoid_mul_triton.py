"""Triton fused ops: sigmoid(x @ w^T) and sigmoid(x @ w^T) * m for w shape (1, H)."""
...
def fused_linear_sigmoid_kernel(
    ...
    """Per row: dot(x[n,:], w[0,:]) -> sigmoid -> store output[n, 0]."""
    ...
    if row_ok:
        tl.store(
            output_ptr + pid_n * output_stride_row,
            sigmoid_x.to(output_ptr.dtype.element_ty),
        )


def fused_linear_sigmoid(
